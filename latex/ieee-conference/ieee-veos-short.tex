...
The EOSEV system is a dual-pipeline framework that supports both training and inference of the deep reinforcement learning model — either locally on the on-board computing units via CAN or Ethernet, or in the cloud via OTA updates, as illustrated in Fig.~\ref{fig:EOSEV_system}. Since the pedal map is only updated when traffic conditions change, the control action Δτ does not require hard real-time guarantees.

An Avatar orchestrates the configuration and scheduling of the states of the dataflow in the CAN/UDP through a KvaserCAN server or via a cloud endpoint. The Cruncher acts as the data hub receiving pipeline data in JSON/DataFrame format from the ECU or the cloud OSS and feeds observations as DataFrames to an Agent, which produces actions back to the Cruncher and exchanges batches, episodes, and DataFrames with a Model that is trained using data from Storage.
