...
\item Benchmark inconsistency, Baselines vary widely: rule-based control, DP, MPC, human driving, or simplified optimal policies, making results hard to compare.
\item Real-time deployment gaps: Computational latency, sensor uncertainty, V2X loss, and certification are under-addressed.
\end{itemize}
...
The EOSEV system is a dual-pipeline framework that supports both training and inference for the deep reinforcement learning model, either locally on the on-board computing units via CAN or Ethernet, or in the cloud through OTA updates, as illustrated in Fig.~\ref{fig:EOSEV_system}. Because the pedal map is updated only when traffic conditions change, the control action $\Delta \tau$ does not require hard real-time guarantees. We therefore run training and inference in the cloud to preserve the current vehicle E/E architecture and share the solution across the product line.

A Scheduler configures and schedules the dataflow states over CAN/UDP, either through a local CAN server (KvaserCAN) or through a cloud endpoint (CloudOSS). The Cruncher acts as the data hub: it receives observations from the ECU or the cloud, forwards them to the Agent, and sends the updated pedal map back to the ECU. The Agent returns the updated pedal map, i.e., the action, to the Cruncher, which stores the observations and actions in Storage and provides observation batches to the Model for training and action generation.
